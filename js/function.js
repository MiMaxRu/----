(function() {
	...
	const getCoordinates = el => {
		...
		return { left: coords.left + window.pageXOffset, right: coords.right + window.pageXOffset, top: coords.top + window.pageYOffset, bottom: coords.bottom + window.pageYOffset };
	};
	...
	class Field {
		...
		static SHIP_SIDE = 33;
		static SHIP_DATA = { fourdeck: [1, 4], tripledeck: [2, 3], doubledeck: [3, 2], singledeck: [4, 1] };

		constructor(field) {
			this.field = field;
			this.squadron = {};
			this.matrix = [];
			const { left, right, top, bottom } = getCoordinates(this.field);
			this.fieldLeft = left;
		...
		static createMatrix = () => [...Array(10)].map(() => Array(10).fill(0));
